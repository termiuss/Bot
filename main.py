...
def get_admin_keyboard():
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="💸 Начислить"), KeyboardButton(text="📝 Добавить заказ")],
            [KeyboardButton(text="🚫 Баны/ограничения"), KeyboardButton(text="💰 Балансы")],
            [KeyboardButton(text="👥 Пользователи"), KeyboardButton(text="📊 Прочее")],
...
def get_users_submenu_keyboard():
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="ℹ️ Информация о пользователях"), KeyboardButton(text="➕ Добавить информацию о пользователе")],
            [KeyboardButton(text="📈 Доход пользователя"), KeyboardButton(text="📊 Экспорт CSV")],
            [KeyboardButton(text="🔙 Назад")],
        ],
        resize_keyboard=True,
        one_time_keyboard=False
        ...
        for squad_id, squad_name in squads:
            ...
            if squad_info:
                ...
                response += (
                    ...
                    f"📋 Заказов:     {total_orders:10}\n"
                    f"💰 Баланс:      {total_balance:10.2f} руб.\n"
                    f"🌟 Рейтинг:     {avg_rating:6.2f} ⭐ ({rating_count} оценок)\n\n"
                )
...
async def action_log(message: types.Message):
    ...
    try:
        ...
        if not logs:
            await message.answer("📜 Журнал действий пуст.", reply_markup=get_misc_submenu_keyboard())
            return
...
async def monthly_report(message: types.Message):
    ...
    try:
        ...
        if not report or report[0] == 0:
            await message.answer("📊 Нет заказов за последние 30 дней.", reply_markup=get_misc_submenu_keyboard())
            return
...
@dp.message(F.text == "ℹ️ Информация о пользователях")
async def view_user_info(message: types.Message, state: FSMContext):
    ...
    if not is_admin(user_id):
        ...
        return
    try:
        await message.answer("ℹ️ Введите Telegram ID пользователя для просмотра информации:", reply_markup=get_cancel_keyboard(True))
...
async def process_view_user_info(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as conn:
            ...
            user = await cursor.fetchone()
            if not user:
                await message.answer(f"⚠️ Пользователь с ID {target_user_id} не найден.", reply_markup=get_users_submenu_keyboard())
                await state.clear()
                return
        ...
        if not info_records:
            await message.answer(f"ℹ️ Для пользователя @{username or 'Unknown'} (ID: {target_user_id}) нет дополнительной информации.", reply_markup=get_users_submenu_keyboard())
        else:
            response = f"ℹ️ Информация о пользователе @{username or 'Unknown'} (ID: {target_user_id}):\n\n"
...
async def export_users_csv(message: types.Message):
    ...
    try:
        ...
        if not users:
            await message.answer("⚠️ Нет данных о пользователях.", reply_markup=get_users_submenu_keyboard())
            return
...
@dp.message(F.text == "➕ Добавить информацию о пользователе")
async def add_user_info(message: types.Message, state: FSMContext):
    ...
    if not is_admin(user_id):
        ...
        return
    try:
        await message.answer("➕ Введите Telegram ID пользователя:", reply_markup=get_cancel_keyboard(True))
...
async def process_add_user_info_id(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("📝 Введите информацию о пользователе:", reply_markup=get_cancel_keyboard(True))
...
async def process_add_user_info_text(message: types.Message, state: FSMContext):
    ...
    if not user_info:
        await message.answer("❌ Информация не может быть пустой. Введите информацию о пользователе:", reply_markup=get_cancel_keyboard(True))
        return
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as conn:
            ...
            user = await cursor.fetchone()
            if not user:
                await message.answer(f"⚠️ Пользователь с ID {target_user_id} не найден в базе данных.", reply_markup=get_users_submenu_keyboard())
                await state.clear()
                return
        ...
        await message.answer(f"✅ Информация о пользователе {target_user_id} добавлена!", reply_markup=get_users_submenu_keyboard())
...
async def list_users(message: types.Message):
    ...
    try:
        ...
        for user in users:
            ...
            response += (
                f"🔹 @{username or 'Unknown':20} (ID: {telegram_id})\n"
                f"🔢 PUBG ID:     {pubg_id or 'не указан':20}\n"
                f"🏠 Сквад:      {squad_name:20}\n"
                f"💰 Баланс:     {balance:10.2f} руб.\n"
                f"⭐ Репутация:  {reputation:10}\n"
                f"📋 Заказов:    {completed_orders:10}\n"
                f"🌟 Рейтинг:    {avg_rating:6.2f} ⭐ ({rating_count} оценок)\n"
                f"🚫 Статус:     {status:20}\n\n"
            )
        await message.answer(response, reply_markup=get_admin_keyboard())
...
async def orders_submenu(message: types.Message):
    if not await check_access(message):
        return
    user_id = message.from_user.id
    try:
        await message.answer("📋 Управление заказами:", reply_markup=get_orders_submenu_keyboard())
    except TelegramAPIError as e:
        logger.error(f"Ошибка Telegram API в orders_submenu для {user_id}: {e}\n{traceback.format_exc()}")
        await message.answer(MESSAGES["error"], reply_markup=get_menu_keyboard(user_id))
...
async def bans_submenu(message: types.Message):
    ...
    if not is_admin(user_id):
        ...
        return
    try:
        await message.answer("🚫 Управление банами и ограничениями:", reply_markup=get_bans_submenu_keyboard())
    except TelegramAPIError as e:
...
async def balances_submenu(message: types.Message):
    ...
    if not is_admin(user_id):
        ...
        return
    try:
        await message.answer("💰 Управление балансами:", reply_markup=get_balances_submenu_keyboard())
    except TelegramAPIError as e:
...
async def misc_submenu(message: types.Message):
    ...
    if not is_admin(user_id):
        ...
        return
    try:
        await message.answer("📊 Прочие функции:", reply_markup=get_misc_submenu_keyboard())
    except TelegramAPIError as e:
