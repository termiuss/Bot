...
def get_users_submenu_keyboard():
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="📈 Доход пользователя")],
            [KeyboardButton(text="🔙 Назад")],
        ],
        resize_keyboard=True,
        one_time_keyboard=False
...
async def export_users_info(message: types.Message):
    user_id = message.from_user.id
    if not is_admin(user_id):
        await message.answer(MESSAGES["no_access"], reply_markup=get_menu_keyboard(user_id))
        return
    try:
        ...
        os.remove(filename)
        await log_action("export_users_info", user_id, None, "Экспортирована информация о пользователях")
    except (aiosqlite.Error, OSError) as e:
        logger.error(f"Ошибка в export_users_info для {user_id}: {e}\n{traceback.format_exc()}")
        await message.answer(MESSAGES["error"], reply_markup=get_users_submenu_keyboard())
    except TelegramAPIError as e:
        logger.error(f"Ошибка Telegram API в export_users_info для {user_id}: {e}\n{traceback.format_exc()}")
        await message.answer(MESSAGES["error"], reply_markup=get_users_submenu_keyboard())
...
async def add_user_info(message: types.Message, state: FSMContext):
    ...
    if not is_admin(user_id):
        ...
        return
    try:
        await message.answer("➕ Добавить информацию о пользователе:", reply_markup=get_cancel_keyboard(True))
...
async def process_add_user_info(message: types.Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text == "🚫 Отмена":
    ...
    try:
        # Здесь можно добавить логику сохранения информации о пользователе
        await message.answer(f"✅ Информация о пользователе добавлена: {user_info}", reply_markup=get_users_submenu_keyboard())
        await log_action("add_user_info", user_id, None, f"Добавлена информация: {user_info}")
        await state.clear()
    except TelegramAPIError as e:
        logger.error(f"Ошибка Telegram API в process_add_user_info для {user_id}: {e}\n{traceback.format_exc()}")
        await message.answer(MESSAGES["error"], reply_markup=get_users_submenu_keyboard())
        await state.clear()
