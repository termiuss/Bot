...
async def process_leader_user_id(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as conn:
            ...
            if current_squad_id:
                # Проверяем, не является ли он уже лидером этого сквада
                cursor = await conn.execute(
                    "SELECT COUNT(*) FROM squad_leaders WHERE leader_id = ? AND squad_id = ?", 
                    (escort_id, current_squad_id)
                )
                is_already_leader = (await cursor.fetchone())[0] > 0
                
                if is_already_leader:
                    await message.answer("❌ Этот пользователь уже является лидером данного сквада.", reply_markup=get_cancel_keyboard(True))
                    return
...
async def list_leaders(message: types.Message):
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as conn:
            cursor = await conn.execute(
                '''
                SELECT s.name as squad_name, 
                       GROUP_CONCAT(e.telegram_id || ':' || COALESCE(e.username, 'Unknown'), ', ') as leaders_info
                FROM squads s
                JOIN squad_leaders sl ON s.id = sl.squad_id
                JOIN escorts e ON sl.leader_id = e.id
                GROUP BY s.id, s.name
                ORDER BY s.name
                '''
            )
            squads_with_leaders = await cursor.fetchall()

        if not squads_with_leaders:
            await message.answer("👑 Список лидеров пуст.", reply_markup=get_leaders_submenu_keyboard())
            return

        response = "👑 Список лидеров по сквадам:\n\n"
        for squad_name, leaders_info in squads_with_leaders:
            response += f"🏠 Сквад: {squad_name}\n"
            response += f"Лидеры:\n"
            for leader in leaders_info.split(', '):
                telegram_id, username = leader.split(':')
...
async def process_remove_leader_user_id(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as conn:
            ...
            cursor = await conn.execute(
                '''
                SELECT e.username, s.name, sl.squad_id
                FROM escorts e
                JOIN squad_leaders sl ON e.id = sl.leader_id
                JOIN squads s ON sl.squad_id = s.id
                WHERE e.id = ?
            ...
            if not leader_info:
                ...
                await state.clear()
                return
            leader_username, squad_name, squad_id = leader_info

            # Удаляем только запись из squad_leaders (убираем статус лидера)
            await conn.execute("DELETE FROM squad_leaders WHERE leader_id = ?", (escort_id_to_remove,))
            
            # Сквад и участники остаются без изменений

            await conn.commit()

        await message.answer(f"👑 Статус лидера снят с @{leader_username or 'Unknown'} (ID: {target_telegram_id}). Сквад '{squad_name}' остался.", reply_markup=get_leaders_submenu_keyboard())
        await log_action("remove_leader", user_id, None, f"Снят статус лидера с {target_telegram_id} (сквад: {squad_name})")
        await state.clear()
