...
async def process_complete_order(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as conn:
            ...
            # Вычисляем комиссию и выплату
            commission = amount * 0.2
            payout_amount = amount - commission
            payout_per_participant = payout_amount / participant_count if participant_count > 0 else 0

            # Начисляем деньги участникам
            await conn.execute(
                '''
                UPDATE escorts 
                SET balance = balance + ? 
            ...
            # Increment completed orders for all participants
            await conn.execute(
                '''
...
async def process_ban_duration(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        try:
            await bot.send_message(telegram_id, f"\n Вы заблокированы до {formatted_date}.")
        except TelegramAPIError:
            pass
        await state.clear()
    except ValueError as e:
...
async def export_users_csv(message: types.Message):
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as conn:
            cursor = await conn.execute(
                ...
                SELECT e.telegram_id, e.username, e.pubg_id, s.name as squad_name, 
                       e.balance, e.reputation, e.completed_orders, e.rating, e.rating_count,
                       e.is_banned, e.ban_until, e.restrict_until, e.rules_accepted
                FROM escorts e
                LEFT JOIN squads s ON e.squad_id = s.id
                '''
            )
        ...
        if not users:
            await message.answer("\n Нет данных о пользователях.", reply_markup=get_users_submenu_keyboard())
            return

        filename = f"users_info_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        with open(filename, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['Telegram ID', 'Username', 'PUBG ID', 'Squad', 'Balance', 'Reputation', 
                           'Completed Orders', 'Rating', 'Rating Count', 'Is Banned', 'Ban Until', 
                           'Restrict Until', 'Rules Accepted'])
            for user in users:
                writer.writerow(user)

        with open(filename, 'rb') as f:
            await message.answer_document(types.FSInputFile(filename, filename), reply_markup=get_users_submenu_keyboard())

        os.remove(filename)
        await log_action("export_users_csv", user_id, None, "Экспортирована информация о пользователях в CSV")
    except (aiosqlite.Error, OSError) as e:
        logger.error(f"Ошибка в export_users_csv для {user_id}: {e}\n\n{traceback.format_exc()}")
        await message.answer(MESSAGES["error"], reply_markup=get_users_submenu_keyboard())
    except TelegramAPIError as e:
        logger.error(f"Ошибка Telegram API в export_users_csv для {user_id}: {e}\n\n{traceback.format_exc()}")
        await message.answer(MESSAGES["error"], reply_markup=get_users_submenu_keyboard())
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as conn:
            cursor = await conn.execute(
                '''
                SELECT e.telegram_id, e.username, e.pubg_id, s.name as squad_name, 
                       e.balance, e.completed_orders, e.rating, e.rating_count
                FROM escorts e
                LEFT JOIN squads s ON e.squad_id = s.id
                ORDER BY e.username
                '''
        ...
        if not users:
            await message.answer("\n Нет зарегистрированных пользователей.", reply_markup=get_users_submenu_keyboard())
            return

        response = "\n Список пользователей:\n\n"
        for telegram_id, username, pubg_id, squad_name, balance, completed_orders, rating, rating_count in users:
            avg_rating = rating / rating_count if rating_count > 0 else 0
            response += (
                f"\n @{username or 'Unknown'} (ID: {telegram_id})\n"
                f"\n PUBG ID: {pubg_id or 'не указан'}\n"
                f"\n Сквад: {squad_name or 'не назначен'}\n"
                f"\n Баланс: {balance:.2f} руб.\n"
                f"\n Заказов: {completed_orders}\n"
                f"\n Рейтинг: {avg_rating:.2f} \n\n\n"
            )
...
async def back_to_menu(message: types.Message):
    ...
    try:
        ...
        elif is_admin(user_id) and current_context in ['admin_panel', 'squads_submenu', 'escorts_submenu', 'bans_submenu', 'balances_submenu', 'misc_submenu', 'users_submenu', 'rating_submenu']:
            await message.answer("\n Админ-панель:", reply_markup=get_admin_keyboard())
            user_context[user_id] = 'admin_panel'
        else:
