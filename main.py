...
def get_admin_keyboard():
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="⭐ Репутация"), KeyboardButton(text="📊 Данные пользователей")],
...
@dp.message(F.text == "📊 Данные пользователей")
async def user_data_submenu(message: types.Message):
    user_id = message.from_user.id
    if not is_admin(user_id):
        await message.answer(MESSAGES["no_access"], reply_markup=get_menu_keyboard(user_id))
        return
    try:
        user_context[user_id] = 'user_data_submenu'
        await message.answer("📊 Управление данными пользователей:", reply_markup=get_user_data_submenu_keyboard())
    except TelegramAPIError as e:
        logger.error(f"Ошибка Telegram API в user_data_submenu для {user_id}: {e}\n\n{traceback.format_exc()}")
        await message.answer(MESSAGES["error"], reply_markup=get_admin_keyboard())

@dp.message(F.text == "📊 Общее КД")
async def general_cd_data(message: types.Message):
    user_id = message.from_user.id
    if not is_admin(user_id):
        await message.answer(MESSAGES["no_access"], reply_markup=get_menu_keyboard(user_id))
        return
    try:
        data = await get_all_user_data_by_type("cd")
        if not data:
            await message.answer("📊 Нет данных КД", reply_markup=get_user_data_submenu_keyboard())
            return

        response = "📊 Общее КД:\n\n"
        for telegram_id, username, value in data:
            response += f"{telegram_id} - @{username or 'Unknown'} - {value}\n"

        await message.answer(response, reply_markup=get_user_data_submenu_keyboard())
    except TelegramAPIError as e:
        logger.error(f"Ошибка Telegram API в general_cd_data для {user_id}: {e}")
        await message.answer(MESSAGES["error"], reply_markup=get_user_data_submenu_keyboard())

@dp.message(F.text == "📊 Общее Айди")
async def general_id_data(message: types.Message):
    user_id = message.from_user.id
    if not is_admin(user_id):
        await message.answer(MESSAGES["no_access"], reply_markup=get_menu_keyboard(user_id))
        return
    try:
        data = await get_all_user_data_by_type("id")
        if not data:
            await message.answer("📊 Нет данных Айди", reply_markup=get_user_data_submenu_keyboard())
            return
        ...
        logger.error(f"Ошибка Telegram API в general_id_data для {user_id}: {e}")
        await message.answer(MESSAGES["error"], reply_markup=get_user_data_submenu_keyboard())

@dp.message(F.text == "📊 Общий Сквад")
async def general_squad_data(message: types.Message):
    user_id = message.from_user.id
    if not is_admin(user_id):
        await message.answer(MESSAGES["no_access"], reply_markup=get_menu_keyboard(user_id))
        return
    try:
        data = await get_all_user_data_by_type("squad")
        if not data:
            await message.answer("📊 Нет данных Сквад", reply_markup=get_user_data_submenu_keyboard())
            return

        response = "📊 Общий Сквад:\n\n"
        for telegram_id, username, value in data:
            response += f"{telegram_id} - @{username or 'Unknown'} - {value}\n"

        await message.answer(response, reply_markup=get_user_data_submenu_keyboard())
    except TelegramAPIError as e:
        logger.error(f"Ошибка Telegram API в general_squad_data для {user_id}: {e}")
        await message.answer(MESSAGES["error"], reply_markup=get_user_data_submenu_keyboard())

@dp.message(F.text == "📊 Общее Прочее")
async def general_other_data(message: types.Message):
    user_id = message.from_user.id
    if not is_admin(user_id):
        await message.answer(MESSAGES["no_access"], reply_markup=get_menu_keyboard(user_id))
        return
    try:
        data = await get_all_user_data_by_type("other")
        if not data:
            await message.answer("📊 Нет прочих данных", reply_markup=get_user_data_submenu_keyboard())
            return

        response = "📊 Общее Прочее:\n\n"
        for telegram_id, username, value in data:
            response += f"{telegram_id} - @{username or 'Unknown'} - {value}\n"

        await message.answer(response, reply_markup=get_user_data_submenu_keyboard())
    except TelegramAPIError as e:
        logger.error(f"Ошибка Telegram API в general_other_data для {user_id}: {e}")
        await message.answer(MESSAGES["error"], reply_markup=get_user_data_submenu_keyboard())
...
async def available_orders(message: types.Message):
    ...
    try:
        ...
        escort = await get_escort(user_id)
        if not escort:
            await message.answer("❌ Ваш профиль не найден.", reply_markup=get_menu_keyboard(user_id))
            return
        
        escort_id, squad_id, pubg_id, _, _, _, _, _, _, _, _, _, _ = escort
        
        if not squad_id:
            await message.answer(MESSAGES["not_in_squad"], reply_markup=get_menu_keyboard(user_id))
            return
        ...
        if not orders:
            await message.answer("📋 Нет доступных заказов для вашего сквада.", reply_markup=get_menu_keyboard(user_id))
            return
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as conn:
            ...
            if not order:
                ...
                return
            if order[0] != 'pending':
                await callback.message.answer(f"⚠️ Заказ #{order[1]} уже начат и его нельзя покинуть.", reply_markup=get_menu_keyboard(user_id))
                await callback.answer()
                return
            
            # Удаляем только текущего пользователя из заявок
            await conn.execute(
            ...
            # Проверяем, остались ли участники в заказе
            cursor = await conn.execute(
        ...
        if remaining_count == 0:
            # Если больше никого нет, просто уведомляем об отмене
            await callback.message.edit_text(f"❌ Вы покинули заказ #{order[1]}", reply_markup=None)
        else:
            # Если остались участники, показываем обновленный список
            await callback.message.edit_text(f"❌ Вы покинули заказ #{order[1]}\nОстальные участники продолжают набор.", reply_markup=None)
        ...
        await log_action(
            ...
            user_id,
            order_db_id,
            f"Пользователь покинул заказ #{order[1]}"
        )
        await callback.answer()
        ...
        logger.error(f"Ошибка Telegram API в leave_order для {user_id}: {e}\n\n{traceback.format_exc()}")
        await callback.message.answer(MESSAGES["error"], reply_markup=get_menu_keyboard(user_id))
        await callback.answer()
...
async def list_users(message: types.Message):
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as conn:
            cursor = await conn.execute(
                '''
                SELECT telegram_id, username, pubg_id, balance, reputation, completed_orders, 
                       is_banned, ban_until, restrict_until, s.name as squad_name
                FROM escorts e
                LEFT JOIN squads s ON e.squad_id = s.id
        ...
        if not users:
            await message.answer("👥 Нет зарегистрированных пользователей.", reply_markup=get_users_submenu_keyboard())
            return
        
        response = "👥 Список пользователей:\n\n"
        for telegram_id, username, pubg_id, balance, reputation, completed_orders, is_banned, ban_until, restrict_until, squad_name in users:
            status = ""
            if is_banned:
                status = " [ЗАБАНЕН]"
            elif ban_until and datetime.fromisoformat(ban_until) > datetime.now():
                status = " [ВРЕМЕННО ЗАБАНЕН]"
            elif restrict_until and datetime.fromisoformat(restrict_until) > datetime.now():
                status = " [ОГРАНИЧЕН]"
            
            response += (
                f"ID: {telegram_id}\n"
                f"@{username or 'Unknown'}{status}\n"
                f"PUBG ID: {pubg_id or 'не указан'}\n"
                f"Сквад: {squad_name or 'не назначен'}\n"
...
async def export_users_csv(message: types.Message):
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as conn:
            cursor = await conn.execute(
                '''
                SELECT e.telegram_id, e.username, e.pubg_id, e.balance, e.reputation, 
                       e.completed_orders, e.is_banned, e.ban_until, e.restrict_until, 
        ...
        if not users:
            await message.answer("📊 Нет данных для экспорта.", reply_markup=get_users_submenu_keyboard())
            return
        
        filename = f"users_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(['Telegram ID', 'Username', 'PUBG ID', 'Balance', 'Reputation', 
                           'Completed Orders', 'Is Banned', 'Ban Until', 'Restrict Until', 'Squad'])
        ...
        os.remove(filename)
        await log_action("export_users_csv", user_id, None, "Экспортированы данные пользователей в CSV")
...
async def process_add_user_info_text(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await log_action("add_user_info", user_id, None, f"Добавлена информация о пользователе {target_user_id}: {user_info}")
        await state.clear()
    except aiosqlite.Error as e:
...
async def cancel_handler(message: types.Message, state: FSMContext):
    ...
    try:
        if is_admin(user_id):
            ...
            if current_context in ['squads_submenu', 'escorts_submenu', 'bans_submenu', 'balances_submenu', 'misc_submenu', 'users_submenu', 'admin_orders_submenu', 'reputation_submenu', 'user_data_submenu']:
                await message.answer(MESSAGES["cancel_action"], reply_markup=get_admin_keyboard())
                user_context[user_id] = 'admin_panel'
...
async def back_handler(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        logger.error(f"Ошибка Telegram API в back_handler для {user_id}: {e}")
        await message.answer("📌 Выберите действие:", reply_markup=get_menu_keyboard(user_id))
