...
async def process_add_member_city(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as conn:
            ...
            await conn.execute(
                """INSERT INTO squad_applications (user_id, squad_id, city, pubg_id, cd, age, status) 
                   VALUES (?, ?, ?, ?, ?, ?, 'approved')""",
                (escort_id, squad_id, city, pubg_id, "не указан", "не указан")
            )
            await conn.commit()
        ...
        logger.error(f"Ошибка в process_add_member_city для {user_id}: {e}")
        await message.answer(MESSAGES["error"], reply_markup=get_members_management_keyboard())
        await state.clear()
...
async def user_rating(message: types.Message):
    ...
    try:
        ...
        if not top_users:
            await message.answer("звезд Пока нет пользователей с рейтингом.", reply_markup=await get_menu_keyboard(user_id))
            return
        
        response = "звезд Топ-10 пользователей по рейтингу:\n\n"
        for i, (username, total_rating, rating_count, completed_orders, telegram_id) in enumerate(top_users, 1):
            ...
            response += f"{i}. @{username or 'Unknown'} - звезд {avg_rating:.2f} ({rating_count} оценок, {completed_orders} заказов){marker}\n"
        
        # Показываем позицию текущего пользователя, если он не в топ-10
        user_position, user_rating_value = await get_user_rating_position(user_id)
        if user_position and user_position > 10:
            response += f"\n📍 Ваша позиция: {user_position} место (звезд {user_rating_value:.2f})"
...
async def squad_rating(message: types.Message):
    ...
    try:
        ...
        for i, (squad_name, avg_rating, member_count, total_orders) in enumerate(top_squads, 1):
            ...
            response += f"{i}. {squad_name} - звезд {avg_rating:.2f} ({member_count} чел., {total_orders} заказов){marker}\n"
        
        # Показываем позицию сквада пользователя, если он не в топ-10
        if user_squad_position and user_squad_position > 10:
            response += f"\n📍 Позиция вашего сквада '{user_squad_name}': {user_squad_position} место (звезд {user_squad_rating:.2f})"
