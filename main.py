...
def get_admin_keyboard():
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="🚫 Баны/ограничения"), KeyboardButton(text="💰 Балансы")],
            [KeyboardButton(text="👥 Пользователи"), KeyboardButton(text="⭐ Рейтинг")],
            [KeyboardButton(text="📊 Прочее"), KeyboardButton(text="🚪 Выйти из админ-панели")],
        ],
        resize_keyboard=True,
        one_time_keyboard=False
    )
...
def get_rating_submenu_keyboard():
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="➕ Добавить рейтинг"), KeyboardButton(text="➖ Снять рейтинг")],
            [KeyboardButton(text="🔙 Назад")],
        ],
        resize_keyboard=True,
        one_time_keyboard=False
...
def get_confirmed_order_keyboard(order_id: str, is_admin: bool = False):
    keyboard_buttons = [[InlineKeyboardButton(text="Завершить заказ", callback_data=f"complete_order_{order_id}")]]
    if is_admin:
        keyboard_buttons.append([InlineKeyboardButton(text="Отменить заказ", callback_data=f"cancel_confirmed_order_{order_id}")])
...
def get_payout_keyboard(payout_id: int):
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ Принять выплату", callback_data=f"accept_payout_{payout_id}"),
            InlineKeyboardButton(text="❌ Отклонить выплату", callback_data=f"reject_payout_{payout_id}")
        ]
    ])
    return keyboard
...
async def info_handler(message: types.Message):
    ...
    try:
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="⭐ О системе рейтинга", url="https://telegra.ph/O-sisteme-rejtinga-08-16")],
...
async def available_orders(message: types.Message):
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as conn:
            cursor = await conn.execute(
                "SELECT id, memo_order_id, customer_info, amount FROM orders WHERE status = 'pending' AND is_completed = 0 AND is_rated = 0"
            )
            orders = await cursor.fetchall()
        if not orders:
        ...
        for db_id, order_id, customer, amount in orders:
            keyboard_buttons.append([InlineKeyboardButton(text=f"#{order_id} - {customer}, {amount:.2f} руб.", callback_data=f"select_order_{db_id}")])
        keyboard = InlineKeyboardMarkup(inline_keyboard=keyboard_buttons)
...
async def my_orders(message: types.Message):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as conn:
            ...
            cursor = await conn.execute(
                '''
                SELECT DISTINCT o.memo_order_id, o.customer_info, o.amount, o.status, o.is_completed, o.is_rated
                FROM orders o
                WHERE o.id IN (
        ...
        if not orders:
            ...
            return
        response = "\n Ваши заказы:\n"
        for order_id, customer, amount, status, is_completed, is_rated in orders:
            status_text = "Ожидает" if status == "pending" else "В процессе" if status == "in_progress" else "Завершен"
            if is_completed and not is_rated:
                status_text += " (Ожидает оценки)"
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as conn:
            ...
            cursor = await conn.execute(
                ...
                JOIN escorts e ON oe.escort_id = e.id
                WHERE o.status = 'in_progress' AND e.telegram_id = ?
                ''', (user_id,)
            )
            orders = await cursor.fetchall()
        if not orders:
...
async def process_complete_order(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as conn:
            ...
            cursor = await conn.execute(
                """
                SELECT o.id, o.status, o.amount, o.squad_id
                FROM orders o
                JOIN order_escorts oe ON o.id = oe.order_id
                JOIN escorts e ON oe.escort_id = e.id
            ...
            if not order:
                ...
                await state.clear()
                return
            order_db_id, _, amount, squad_id = order
            commission = amount * 0.2
            payout_amount = (amount - commission)

            # Получаем количество участников заказа
            cursor = await conn.execute(
            ...
            if participant_count > 0:
                payout_per_participant = payout_amount / participant_count

                # Начисляем деньги и 200 рейтинга участникам
                await conn.execute(
                    '''
                    UPDATE escorts
                    SET balance = balance + ?, reputation = reputation + 200
                    WHERE id IN (
            ...
            # Обновляем статус заказа на 'completed' и устанавливаем флаги
            await conn.execute(
                '''
                UPDATE orders SET status = 'completed', completed_at = ?, is_completed = 1 WHERE id = ?
                ''', (datetime.now().isoformat(), order_db_id)
            )
            await conn.execute(
                '''
                UPDATE escorts SET completed_orders = completed_orders + 1
                WHERE id IN (
                    SELECT escort_id FROM order_escorts WHERE order_id = ?
                )
                ''', (order_db_id,)
        ...
        await notify_admins(
            MESSAGES["order_completed"].format(
                ...
                telegram_id=user_id,
                pubg_id=pubg_id or "не указан"
            ) + f"\n\nКомиссия: {commission:.2f} руб.\nВыплачено участникам: {payout_amount:.2f} руб."
        )

        await log_action("complete_order", user_id, order_db_id, f"Заказ #{order_id} завершен пользователем @{username}, начислено {payout_amount:.2f} руб.")
...
async def rate_order_callback(callback: types.CallbackQuery):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as conn:
            cursor = await conn.execute(
                '''
                SELECT o.id, o.squad_id, o.is_rated
                FROM orders o
                JOIN order_escorts oe ON o.id = oe.order_id
                JOIN escorts e ON oe.escort_id = e.id
            ...
            order = await cursor.fetchone()
            if not order:
                await callback.message.answer("\n Заказ не найден или не завершен.", reply_markup=get_menu_keyboard(user_id))
                await callback.answer()
                return
            order_db_id, squad_id, is_rated = order

            if is_rated:
                await callback.message.answer("⚠️ Этот заказ уже оценен.", reply_markup=get_menu_keyboard(user_id))
                await callback.answer()
                return

            # Начисляем бонусный рейтинг
            bonus_rating = {1: 100, 2: 200, 3: 300, 4: 400, 5: 500}.get(rating, 0)
            if bonus_rating > 0:
                await conn.execute(
                    '''
                    UPDATE escorts
                    SET reputation = reputation + ?, balance = balance + ?
                    WHERE id IN (SELECT escort_id FROM order_escorts WHERE order_id = ?)
                    ''', (bonus_rating, bonus_rating, order_db_id)
                )

            # Сохраняем оценку и обновляем флаг is_rated
            await conn.execute(
                "INSERT INTO order_ratings (order_id, rating, user_id) VALUES (?, ?, ?)",
                (order_db_id, rating, user_id)
            )
            await conn.execute(
                "UPDATE orders SET is_rated = 1 WHERE id = ?",
                (order_db_id,)
            )

            # Обновляем средний рейтинг сквада, если он есть
            if squad_id:
...
@dp.callback_query(F.data.startswith("accept_payout_"))
async def accept_payout(callback: types.CallbackQuery):
    user_id = callback.from_user.id
    if not is_admin(user_id):
        await callback.answer("У вас нет прав на выполнение этого действия.")
        return
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as conn:
            ...
            cursor = await conn.execute(
                "SELECT escort_id, amount FROM pending_payouts WHERE id = ?",
                (payout_id,)
            )
            payout_request = await cursor.fetchone()
            if not payout_request:
                await callback.message.answer("Запрос на выплату не найден.")
                await callback.answer()
                return

            escort_id, amount = payout_request

            # Получаем информацию о пользователе
            cursor = await conn.execute(
                "SELECT telegram_id, username FROM escorts WHERE id = ?",
                (escort_id,)
            )
            escort_info = await cursor.fetchone()
            if not escort_info:
                await callback.message.answer("Информация о пользователе не найдена.")
                await callback.answer()
                return
            telegram_id, username = escort_info
            ...
            await conn.execute(
                "INSERT INTO payouts (escort_id, amount, payout_date) VALUES (?, ?, ?)",
                (escort_id, amount, datetime.now().isoformat())
            )
            # Удаляем запрос из pending_payouts
            await conn.execute("DELETE FROM pending_payouts WHERE id = ?", (payout_id,))
            await conn.commit()

        await callback.message.edit_text(f"✅ Выплата в размере {amount:.2f} руб. одобрена и отправлена пользователю @{username}.")
        await bot.send_message(telegram_id, f"✅ Ваша выплата в размере {amount:.2f} руб. одобрена!")
        await log_action("accept_payout", user_id, None, f"Выплата {amount:.2f} пользователю @{username} одобрена")
        await callback.answer()

    except (ValueError, aiosqlite.Error) as e:
        logger.error(f"Ошибка в accept_payout для {user_id}: {e}\n\n{traceback.format_exc()}")
        await callback.message.answer(MESSAGES["error"])
        await callback.answer()
    except TelegramAPIError as e:
        logger.error(f"Ошибка Telegram API в accept_payout для {user_id}: {e}\n\n{traceback.format_exc()}")
        await callback.message.answer(MESSAGES["error"])
        await callback.answer()
...
async def reject_payout(callback: types.CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(f"❌ Запрос на выплату в размере {amount:.2f} руб. отклонен. Средства возвращены пользователю @{username}.")
        await bot.send_message(telegram_id, f"❌ Ваш запрос на выплату в размере {amount:.2f} руб. был отклонен.")
        await log_action("reject_payout", user_id, None, f"Запрос на выплату {amount:.2f} пользователю @{username} отклонен")
        await callback.answer()
        ...
        logger.error(f"Ошибка Telegram API в reject_payout для {user_id}: {e}\n\n{traceback.format_exc()}")
        await callback.message.answer(MESSAGES["error"])
        await callback.answer()
...
async def process_payout_request(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as conn:
            # Создаем запрос на выплату без списания с баланса
            cursor = await conn.execute(
                '''
                INSERT INTO pending_payouts (escort_id, amount)
                VALUES (?, ?)
                ''', (escort_id, payout_amount)
            )
            payout_id = cursor.lastrowid
            await conn.commit()

        await message.answer(
            f"📥 Запрос на выплату {payout_amount:.2f} руб. отправлен администраторам!\n\n"
            f"Просьба связаться с администратором @ItMEMOO\n"
            f"Или @MemoSpamBlock_bot",
            reply_markup=get_menu_keyboard(user_id)
        )
        await notify_admins(
            f"📥 Запрос выплаты от @{username or 'Unknown'} (ID: {user_id}) на сумму {payout_amount:.2f} руб.",
            reply_markup=get_payout_keyboard(payout_id)
        )
        await log_action(
            "payout_request",
            user_id,
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as conn:
            ...
            if participant_count >= 6:
                await callback.message.answer("⚠️ Максимум 6 участников для заказа!", reply_markup=get_menu_keyboard(user_id))
                await callback.answer()
                return
            await conn.execute(
        ...
        response += f"Участников: {len(applications)}/6"
...
async def start_order(callback: types.CallbackQuery):
    ...
    try:
        ...
        keyboard = get_confirmed_order_keyboard(order_id, is_admin(user_id))
        await callback.message.edit_text(response, reply_markup=keyboard)
...
async def complete_order_callback(callback: types.CallbackQuery):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as conn:
            cursor = await conn.execute(
                "SELECT id, status, amount, squad_id FROM orders WHERE memo_order_id = ? AND status = 'in_progress'",
                (memo_order_id,)
            )
            order = await cursor.fetchone()
            if not order:
...
async def cancel_confirmed_order(callback: types.CallbackQuery):
    ...
    if not is_admin(user_id):
        await callback.answer("У вас нет прав на отмену заказа.")
        return
...
async def process_escort_info(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if not text or text.count(",") != 3:
            await message.answer("❌ Неверный формат. Используйте: telegram_id, @username, pubg_id, squad_name", reply_markup=get_cancel_keyboard(True))
            return
            
        data = [x.strip() for x in text.split(",")]
        if len(data) != 4 or not all(data):
            await message.answer("❌ Все поля должны быть заполнены", reply_markup=get_cancel_keyboard(True))
            return
            
        telegram_id_str, username, pubg_id, squad_name = data
        ...
        try:
            ...
            await message.answer("❌ Telegram ID должен быть числом", reply_markup=get_cancel_keyboard(True))
            return
            
        # Очищаем username от @
        username = username.lstrip("@")
        if not username:
            await message.answer("❌ Username не может быть пустым", reply_markup=get_cancel_keyboard(True))
            return
            
        # Проверяем PUBG ID
        if not pubg_id or len(pubg_id) < 3:
            await message.answer("❌ PUBG ID должен содержать минимум 3 символа", reply_markup=get_cancel_keyboard(True))
            return
            
        async with aiosqlite.connect(DB_PATH) as conn:
            # Проверяем существование сквада
            cursor = await conn.execute("SELECT id FROM squads WHERE name = ?", (squad_name,))
            squad = await cursor.fetchone()
            if not squad:
            ...
            # Проверяем количество участников в скваде
            cursor = await conn.execute("SELECT COUNT(*) FROM escorts WHERE squad_id = ?", (squad_id,))
            squad_member_count = (await cursor.fetchone())[0]
            
            # Проверяем, есть ли уже такой пользователь
            cursor = await conn.execute("SELECT id, squad_id FROM escorts WHERE telegram_id = ?", (telegram_id,))
            existing_escort = await cursor.fetchone()
            
            # Если пользователь уже в другом скваде и новый сквад полный
            if existing_escort and existing_escort[1] != squad_id and squad_member_count >= 6:
                await message.answer(f"❌ Сквад '{squad_name}' уже имеет максимум 6 участников!", reply_markup=get_cancel_keyboard(True))
                return
            
            # Если новый пользователь и сквад полный
            if not existing_escort and squad_member_count >= 6:
                await message.answer(f"❌ Сквад '{squad_name}' уже имеет максимум 6 участников!", reply_markup=get_cancel_keyboard(True))
                return
        ...
        await message.answer(
            f"✅ Сопровождающий @{username} {action_text} в сквад '{squad_name}'!", 
            reply_markup=get_admin_keyboard()
        )
        await log_action(
            "add_escort", user_id, None, f"Сопровождающий @{username} {action_text} в сквад '{squad_name}'"
        )
        await state.clear()
        
    except ValueError as e:
        logger.error(f"Ошибка преобразования данных в process_escort_info для {user_id}: {e}\n{traceback.format_exc()}")
        await message.answer("❌ Ошибка обработки данных. Проверьте формат ввода.", reply_markup=get_cancel_keyboard(True))
    except aiosqlite.Error as e:
        ...
        logger.error(f"Неожиданная ошибка в process_escort_info для {user_id}: {e}\n\n{traceback.format_exc()}")
        await message.answer(MESSAGES["error"], reply_markup=get_admin_keyboard())
        await state.clear()
...
async def rating_submenu_handler(message: types.Message):
    user_id = message.from_user.id
    if not is_admin(user_id):
        await message.answer(MESSAGES["no_access"], reply_markup=get_menu_keyboard(user_id))
        return
    try:
        user_context[user_id] = 'rating_submenu'
        await message.answer("⭐ Управление рейтингом:", reply_markup=get_rating_submenu_keyboard())
    except TelegramAPIError as e:
        logger.error(f"Ошибка Telegram API в rating_submenu_handler для {user_id}: {e}\n\n{traceback.format_exc()}")
        await message.answer(MESSAGES["error"], reply_markup=get_admin_keyboard())

@dp.message(F.text == "➕ Добавить рейтинг")
async def add_rating_handler(message: types.Message, state: FSMContext):
    user_id = message.from_user.id
    if not is_admin(user_id):
        await message.answer(MESSAGES["no_access"], reply_markup=get_menu_keyboard(user_id))
        return
    try:
        await message.answer(
            "➕ Введите Telegram ID пользователя и количество рейтинга для добавления через запятую\n"
            "Пример: 123456789, 5",
            reply_markup=get_cancel_keyboard(True)
        )
        await state.set_state(Form.add_rating)
    except TelegramAPIError as e:
        logger.error(f"Ошибка Telegram API в add_rating_handler для {user_id}: {e}\n\n{traceback.format_exc()}")
        await message.answer(MESSAGES["error"], reply_markup=get_rating_submenu_keyboard())
        await state.clear()

@dp.message(Form.add_rating)
async def process_add_rating(message: types.Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text == "🚫 Отмена":
        await message.answer(MESSAGES["cancel_action"], reply_markup=get_rating_submenu_keyboard())
        await state.clear()
        return
    ...
    try:
        ...
        if len(data) != 2:
            await message.answer("❌ Неверный формат. Используйте: Telegram_ID, количество_рейтинга", reply_markup=get_cancel_keyboard(True))
            return

        telegram_id, rating_amount = data
        telegram_id = int(telegram_id)
        rating_amount = int(rating_amount)

        if rating_amount <= 0:
            await message.answer("❌ Количество рейтинга должно быть больше 0", reply_markup=get_cancel_keyboard(True))
            return

        async with aiosqlite.connect(DB_PATH) as conn:
            cursor = await conn.execute("SELECT id, username, rating, rating_count FROM escorts WHERE telegram_id = ?", (telegram_id,))
            escort = await cursor.fetchone()
            ...
            escort_id, username, current_rating, rating_count = escort
            new_rating_count = rating_count + 1
            new_rating = (current_rating * rating_count + rating_amount) / new_rating_count

            await conn.execute(
                "UPDATE escorts SET rating = ?, rating_count = ?, reputation = reputation + ? WHERE id = ?",
                (new_rating, new_rating_count, rating_amount, escort_id)
            )
            await conn.commit()

        await message.answer(
            f"✅ Рейтинг {rating_amount} успешно добавлен пользователю @{username or 'Unknown'} (ID: {telegram_id})\n"
            f"Новый средний рейтинг: {new_rating:.2f}",
            reply_markup=get_rating_submenu_keyboard()
        )

        await log_action("add_rating", user_id, None, f"Добавлен рейтинг {rating_amount} пользователю @{username or 'Unknown'}")
        await state.clear()

    except ValueError:
        await message.answer("❌ Неверный формат данных. Проверьте правильность ввода.", reply_markup=get_cancel_keyboard(True))
    except aiosqlite.Error as e:
        logger.error(f"Ошибка базы данных в process_add_rating для {user_id}: {e}\n\n{traceback.format_exc()}")
        await message.answer(MESSAGES["error"], reply_markup=get_rating_submenu_keyboard())
        await state.clear()
    except TelegramAPIError as e:
        logger.error(f"Ошибка Telegram API в process_add_rating для {user_id}: {e}\n\n{traceback.format_exc()}")
        await message.answer(MESSAGES["error"], reply_markup=get_rating_submenu_keyboard())
        await state.clear()

@dp.message(F.text == "➖ Снять рейтинг")
async def remove_rating_handler(message: types.Message, state: FSMContext):
    user_id = message.from_user.id
    if not is_admin(user_id):
        await message.answer(MESSAGES["no_access"], reply_markup=get_menu_keyboard(user_id))
        return
    try:
        await message.answer(
            "➖ Введите Telegram ID пользователя и количество рейтинга для снятия через запятую\n"
            "Пример: 123456789, 3",
            reply_markup=get_cancel_keyboard(True)
        )
        await state.set_state(Form.remove_rating)
    except TelegramAPIError as e:
        logger.error(f"Ошибка Telegram API в remove_rating_handler для {user_id}: {e}\n\n{traceback.format_exc()}")
        await message.answer(MESSAGES["error"], reply_markup=get_rating_submenu_keyboard())
        await state.clear()

@dp.message(Form.remove_rating)
async def process_remove_rating(message: types.Message, state: FSMContext):
    user_id = message.from_user.id
    if message.text == "🚫 Отмена":
        await message.answer(MESSAGES["cancel_action"], reply_markup=get_rating_submenu_keyboard())
        await state.clear()
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as conn:
            cursor = await conn.execute("SELECT id, username, reputation FROM escorts WHERE telegram_id = ?", (telegram_id,))
            escort = await cursor.fetchone()
            ...
            escort_id, username, current_reputation = escort
            new_reputation = max(0, current_reputation - rating_amount)  # Не даем уйти в минус

            await conn.execute(
                "UPDATE escorts SET reputation = ? WHERE id = ?",
                (new_reputation, escort_id)
            )
            await conn.commit()

        await message.answer(
            f"✅ Рейтинг {rating_amount} успешно снят с пользователя @{username or 'Unknown'} (ID: {telegram_id})\n"
            f"Новая репутация: {new_reputation}",
            reply_markup=get_rating_submenu_keyboard()
        )

        await log_action("remove_rating", user_id, None, f"Снят рейтинг {rating_amount} с пользователя @{username or 'Unknown'}")
        await state.clear()

    except ValueError:
        await message.answer("❌ Неверный формат данных. Проверьте правильность ввода.", reply_markup=get_cancel_keyboard(True))
    except aiosqlite.Error as e:
        logger.error(f"Ошибка базы данных в process_remove_rating для {user_id}: {e}\n\n{traceback.format_exc()}")
        await message.answer(MESSAGES["error"], reply_markup=get_rating_submenu_keyboard())
        await state.clear()
    except TelegramAPIError as e:
        logger.error(f"Ошибка Telegram API в process_remove_rating для {user_id}: {e}\n\n{traceback.format_exc()}")
        await message.answer(MESSAGES["error"], reply_markup=get_rating_submenu_keyboard())
        await state.clear()
...
async def back_to_menu(message: types.Message):
    ...
    try:
        ...
        elif is_admin(user_id) and current_context in ['admin_panel', 'squads_submenu', 'escorts_submenu', 'bans_submenu', 'balances_submenu', 'misc_submenu', 'users_submenu', 'rating_submenu', 'orders_admin_submenu']:
