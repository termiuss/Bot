...
async def show_order_participants_menu(message, order_db_id: int, memo_order_id: str):
    ...
    try:
        ...
        if len(participants) < 2:
            response += f"\n⚠️ Минимум участников: 2 (сейчас: {len(participants)})"
        elif len(participants) >= 4:
            response += f"\n✅ Набор полный! Можно начинать выполнение."
        else:
            response += f"\n✅ Можно начинать выполнение или добавить ещё участников."
...
async def select_order(callback: types.CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(order_text, reply_markup=get_order_keyboard(order_db_id))
        await callback.answer()
