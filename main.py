...
async def info_handler(message: types.Message):
    ...
    try:
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="🔗 Система репутации", url="https://telegra.ph/Sistema-reputacii-08-17-2")] # Добавлена кнопка
        ])
        response = (
...
async def process_complete_order(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as conn:
            ...
            # Increment completed orders and add base rating for all participants
            await conn.execute(
                '''
                UPDATE escorts SET completed_orders = completed_orders + 1, reputation = reputation + 200
                WHERE id IN (
...
async def back_to_menu(message: types.Message):
    ...
    try:
        ...
        elif is_admin(user_id) and current_context in ['admin_panel', 'squads_submenu', 'escorts_submenu', 'bans_submenu', 'balances_submenu', 'misc_submenu', 'users_submenu', 'admin_orders_submenu']:
            await message.answer("\n Админ-панель:", reply_markup=get_admin_keyboard())
            user_context[user_id] = 'admin_panel'
        else:
