...
def get_users_submenu_keyboard():
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="👥 Пользователи"), KeyboardButton(text="📈 Доход пользователя")],
            [KeyboardButton(text="ℹ️ Информация о пользователе"), KeyboardButton(text="ℹ️ Информация о пользователях")],
            [KeyboardButton(text="🔙 Назад")],
        ],
        resize_keyboard=True,
        one_time_keyboard=False
...
@dp.message(F.text.in_(["ℹ️ Информация о пользователе", "ℹ️ Информация о пользователях"]))
async def user_info_handler(message: types.Message):
    ...
    if not is_admin(user_id):
        ...
        return
    try:
        if message.text == "ℹ️ Информация о пользователе":
            await message.answer("ℹ️ Функция получения информации о конкретном пользователе в разработке.", reply_markup=get_users_submenu_keyboard())
        else:
            await message.answer("ℹ️ Функция получения информации о всех пользователях в разработке.", reply_markup=get_users_submenu_keyboard())
    except TelegramAPIError as e:
        logger.error(f"Ошибка Telegram API в user_info_handler для {user_id}: {e}\n{traceback.format_exc()}")
