...
def get_confirmed_order_keyboard(order_id: str):
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="Отменить заказ", callback_data=f"cancel_confirmed_order_{order_id}")]
    ])
    return keyboard
...
(order_db_id,)
            ...
            if participant_count < 4:
                await message.answer(f"\n Недостаточно сопровождающих для завершения заказа (требуется 4, есть {participant_count}).", reply_markup=get_menu_keyboard(user_id))
                await state.clear()
                return
        ...
        # Автоматически предлагаем админам оценить заказ
        for admin_id in ADMIN_IDS:
            try:
                rating_keyboard = get_rating_keyboard(order_id)
                await bot.send_message(
                    admin_id,
                    f"🌟 Заказ #{order_id} завершен!\nПоставьте оценку за выполнение заказа:",
                    reply_markup=rating_keyboard
                )
            except TelegramAPIError as e:
                logger.warning(f"Не удалось отправить предложение оценки админу {admin_id}: {e}")
...
async def rate_order_start(message: types.Message, state: FSMContext):
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as conn:
            ...
            orders = await cursor.fetchall()
        if not orders:
            await message.answer("\n Нет заказов для оценки.", reply_markup=get_menu_keyboard(user_id))
            await state.clear()
            return
        response = "\n Введите ID заказа для оценки:\n"
        for order_id, _, _, amount in orders:
            response += f"#{order_id} - {amount:.2f} руб.\n"
        await message.answer(response, reply_markup=get_cancel_keyboard())
...
async def process_rate_order(message: types.Message, state: FSMContext):
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as conn:
            ...
            order = await cursor.fetchone()
            if not order:
                await message.answer("\n Заказ не найден, не завершен или уже оценен.", reply_markup=get_menu_keyboard(user_id))
                await state.clear()
                return
...
async def start_order(callback: types.CallbackQuery):
    ...
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as conn:
            ...
            if len(applications) != 4:  # Требуется ровно 4 участника
...
async def complete_order_callback(callback: types.CallbackQuery):
    ...
    try:
        ...
        for admin_id in ADMIN_IDS:
            try:
                rating_keyboard = get_rating_keyboard(memo_order_id)
                await bot.send_message(
                    admin_id,
...
async def back_to_menu(message: types.Message):
    ...
    try:
        ...
        elif is_admin(user_id) and current_context in ['admin_panel', 'squads_submenu', 'escorts_submenu', 'bans_submenu', 'balances_submenu', 'misc_submenu', 'users_submenu']:
            await message.answer("\n Админ-панель:", reply_markup=get_admin_keyboard())
            user_context[user_id] = 'admin_panel'
        else:
