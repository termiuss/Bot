...
async def available_orders(message: types.Message):
    ...
    try:
        ...
        # Если пользователь не в скваде, показываем все заказы с предупреждением
        async with aiosqlite.connect(DB_PATH) as conn:
            cursor = await conn.execute(
                '''
        ...
        if not all_orders:
            await message.answer("📋 Нет доступных заказов.", reply_markup=get_orders_submenu_keyboard())
            return

        if not squad_id:
            # Для пользователей без сквада показываем все заказы, но без возможности присоединения
            ...
            response += "⚠️ Для участия в заказах необходимо состоять в скваде!"
            await message.answer(response, reply_markup=get_orders_submenu_keyboard())
            return

        # Для пользователей со сквадом - фильтруем заказы
